import { selectApiBaseUrl, selectIsFeatureEnabled, selectFeatures } from './app-config.selectors';
...
describe('appConfig selectors', () => {
  ...
  it('selectApiBaseUrl returns api base url', () => {
    expect(selectApiBaseUrl(buildRootState({ apiBaseUrl: 'https://api.example.com' }))).toBe(
  ...
  it('selectIsFeatureEnabled returns false for missing keys', () => {
    expect(
      selectIsFeatureEnabled('unknown')(buildRootState({ features: { analytics: true } })),
    ).toBe(false);
  });
